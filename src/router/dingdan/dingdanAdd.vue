<template>
	<div>
		...
		<el-table stripe show-header border ref="multipleTable" :data="tableData1" tooltip-effect="dark" :fit="true" style="width:100%">
			<el-table-column label="原片">
				...
				<el-table-column label="石料" width="150">
					<template slot-scope="scope">
						<el-select v-model="scope.row.stoneType" placeholder="选择名称" @change="changePrice(priceList,scope.row.delname,scope.$index)">
							<el-option v-for="item in priceList" :key="item.name" :label="item.name" :value="item.name"></el-option>
						</el-select>
					</template>
				</el-table-column>

				<el-table-column label="原片数量" width="210">
					<template slot-scope="scope">
						<el-input-number :min="0" v-model="scope.row.rawNumber" @change="totleEMB"></el-input-number>
					</template>
				</el-table-column>
			</el-table-column>
			<el-table-column label="成品">
				<el-table-column label="加工长" width="210">
					<template slot-scope="scope">
						<div style="padding: 10px 0;" v-for="item in scope.row.routineInfo">
							<el-input-number :min="0" v-model="item.productLength"></el-input-number>
						</div>
					</template>
				</el-table-column>
				<el-table-column label="加工高" width="210">
					<template slot-scope="scope">
						<div style="padding: 10px 0;" v-for="item in scope.row.routineInfo">
							<el-input-number :min="0" v-model="item.productWidth"></el-input-number>
						</div>
					</template>
				</el-table-column>
				<el-table-column label="加工数量" width="210">
					<template slot-scope="scope">
						<div style="padding: 10px 0;" v-for="item in scope.row.routineInfo">
							<el-input-number :min="0" v-model="item.productNumber"></el-input-number>
						</div>
					</template>
				</el-table-column>
				<el-table-column label="加工需求" width="300">
					<template slot-scope="scope">
						<div style="padding: 10px 0;" v-for="item in scope.row.routineInfo">
							<!--做查看编辑-->
							<!--<el-checkbox-group v-model="checkedCities" @change="handleCheckedCitiesChange">-->
							<el-checkbox-group v-model="item.priceConfigIds" @change="handleCheckedCitiesChange">
								<el-checkbox v-for="city in item.priceConfigIds" :label="city" :key="city">{{city}}</el-checkbox>
							</el-checkbox-group>
						</div>
					</template>
				</el-table-column>
				...
				<el-table-column label="加工示意图" width="200">
					<template slot-scope="scope">
						<div style="padding: 10px 0;" v-for="(item,sindex) in scope.row.routineInfo">
							<div class='uc-upload'>
								<div :id="uploaderFiles.dropId" :class="uploaderFiles.showTip ? 'error' : ''">
									<span @click="uploade(1,scope.$index,sindex)">选择文件</span>
								</div>
								<span v-show="uploaderFiles.showTip">{{uploaderFiles.tips}}</span>
								<section>
									<div v-for="(item1, index) in item.picture">
										<i class="fa fa-times" aria-hidden="true" @click="removeFile(1,scope.$index,index)"></i>
										<img :src="item1" @click="scaleImg(2, item1)">
										<div :style="item1.style"></div>
									</div>
								</section>
				...
				<el-table-column label="操作成品" width="100">
					<template slot-scope="scope">
						<div style="padding: 10px 0;" v-for="(item,index) in scope.row.routineInfo">
							<el-button type="danger" v-if="index!=0" @click="delCp(scope.$index,index)">删除</el-button>
							<el-button type="success" v-if="index==0" @click="addCp(scope.$index)">增加</el-button>
						</div>
					</template>
				</el-table-column>

			</el-table-column>
			<el-table-column label="操作加工信息" width="160">
				<template slot-scope="scope">
					<el-button type="primary" v-if="scope.$index==0" @click="addBasicInfo()">添加基本信息</el-button>
		...
		data() {
			return {
				...
				tableData1: [
					...
					{
						"name": "",
						"stoneType": "", //石料类别：1=微晶石、2=全抛釉、3=玻化砖、4=特殊砖
						"rawSizeType": "",
						"rawNumber": "",
						"routineInfo": [{
								"priceConfigIds": [],
								...
								"remark": "",
								"picture": [
//									"http://ouvvk5b6m.bkt.clouddn.com/quchong_2017101309328",
//									"http://ouvvk5b6m.bkt.clouddn.com/quchong_2017101309328"
								]
							},
						],
		...
		methods: {
			...
			uploade(type, num,sindex) {
				this.type = type;
				this.num = num;
				this.sindex = sindex; //成品索引
			...
			addBasicInfo() {
				this.tableData1.push({
					...
					//					price: 0, //价格
					"name": "原片",
					"stoneType": 2, //石料类别：1=微晶石、2=全抛釉、3=玻化砖、4=特殊砖
					"rawSizeType": 2,
					"rawNumber": 4,
					"routineInfo": [{
							"priceConfigIds": ["1", "2"],
			...
			delCp($index, index) {
				alert($index);
				alert(index);
				this.tableData1[$index].routineInfo.splice(index, 1);
			},
			//增加成品
			addCp($index) {
				this.tableData1[$index].routineInfo.push({
					"priceConfigIds": [],
					"productLength": "",
					"productWidth": "",
					"productNumber": "",
						...
						'FileUploaded': function(up, file, info) {
							...
							for(let item of _this.uploaderFiles.files) {
								if(item.id === file.id) {
									item.url = localLink;
									if(_this.type == 1) {
										_this.tableData1[_this.num].routineInfo[_this.sindex].picture.push(_this.uploaderFiles.files[0].url);
									} else {
